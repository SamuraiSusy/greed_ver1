...
void HitCheck::distanceToBorder(GameObject *hitbox, GameObject *character, vec2 *whereToMove)
{
	...
	// Down
	if(difference.y < -abs(difference.x))
	{
...
//vec2 HitCheck::CheckHit(GameObject* LightObject, GameObject* HeavyObject, vec2 Direction)
//{
//
//	//Check hits only if close enough to hit
//	if ((LightObject->getPosition() - HeavyObject->getPosition()).Length() < (LightObject->getSize() + HeavyObject->getSize()).Length()/2)
//	{
//		vec2 direction(0);
//
//		//if(ObjectA->isInside(ObjectB->getPosition()))
//			printf("Hit");
//
//		return direction;
//	}
